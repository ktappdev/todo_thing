...
import { useTasks, useMe, useToggleTaskCompletion } from '../../hooks/useApi';
...
const TaskListScreen = () => {
  ...
  const { data: tasks = [], isLoading: isTasksLoading, refetch } = useTasks(householdId ?? '');
  const toggleTaskMutation = useToggleTaskCompletion(householdId ?? '');
  const isConnected = false as boolean;
  const status = 'disconnected' as const;

  const [filteredTasks, setFilteredTasks] = useState<Task[]>([]);
  const [useRawList, setUseRawList] = useState<boolean>(true);
    ...
    >
      ...
      >
        <Text style={[
          styles.statusText,
          item.completed && styles.completedText,
          toggleTaskMutation.isPending && styles.pendingText,
        ]}>
          {toggleTaskMutation.isPending ? '⏳' : (item.completed ? '✓' : '○')}
        </Text>
      </TouchableOpacity>
    </TouchableOpacity>
  ), [navigation, toggleTaskMutation.isPending]);
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={useRawList ? tasks : filteredTasks}
      ...
      {/* Temporary toggle to help isolate loop source */}
      {/* <TouchableOpacity style={{padding:12}} onPress={() => setUseRawList(v=>!v)}><Text>Toggle Filtered</Text></TouchableOpacity> */}
